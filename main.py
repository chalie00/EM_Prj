...
def normalize_import_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if header_row is None:
        raise ValueError("헤더 행(쿠폰번호)을 찾을 수 없습니다.")
...
class App:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top_bar, text="원본 불러오기", command=self.browse_excel).grid(row=0, column=0, padx=4)
        ttk.Button(top_bar, text="사진 선택 (다중)", command=self.browse_photos).grid(row=0, column=1, padx=4)
        ttk.Label(top_bar, text="Grid:").grid(row=0, column=2, padx=(12, 2))
        self.cols_var = tk.IntVar(value=2)
        self.rows_var = tk.IntVar(value=3)
        ttk.Spinbox(top_bar, from_=1, to=5, textvariable=self.cols_var, width=4).grid(row=0, column=3)
        ttk.Label(top_bar, text="x").grid(row=0, column=4)
        ttk.Spinbox(top_bar, from_=1, to=5, textvariable=self.rows_var, width=4).grid(row=0, column=5)
        ttk.Button(top_bar, text="사진 분석", command=self.run_analysis).grid(row=0, column=6, padx=8)
        ttk.Button(top_bar, text="내보내기", command=self.export_excel).grid(row=0, column=7, padx=8)
        ttk.Button(top_bar, text="폴더 열기", command=self.open_export_folder).grid(row=0, column=8, padx=8)
        self.status_var = tk.StringVar(value="준비 완료")
        ttk.Label(top_bar, textvariable=self.status_var).grid(row=0, column=9, sticky="e")
        ...
        ttk.Label(main_frame, text="원본 파일").grid(row=0, column=0, columnspan=2, sticky="w")
    ...
    def run_analysis(self) -> None:
        ...
        if not records:
            messagebox.showinfo("정보", "추출된 쿠폰이 없습니다.")
            return
    ...
    def open_export_folder(self) -> None:
        if not self._last_export_path:
            messagebox.showinfo("정보", "Export된 파일이 없습니다.")
            return
        folder = self._last_export_path.parent
        if folder.exists():
    ...
    def _show_missing(self) -> None:
        ...
        for _, row in self.ocr_df.iterrows():
            ...
            if not num or num not in base_numbers:
                missing_rows.append(f"{row.get('이미지','')}, 위치 {row.get('위치','')}, 쿠폰번호: {num or '인식 실패'}")
        if missing_rows:
    ...
    def export_excel(self) -> None:
        if self.import_df is None:
            ...
            return
        if self.ocr_df is None:
            messagebox.showwarning("경고", "먼저 OCR Analysis 를 실행하세요.")
            return
        if self._export_in_progress:
            messagebox.showinfo("정보", "Export가 진행 중입니다. 잠시만 기다려주세요.")
            return
        ...
        def worker() -> None:
            ...
            try:
                ...
                log_path.write_text(
                    f"replaced temp -> target\n",
                    encoding="utf-8",
                    errors="replace",
                )
            except PermissionError:
                error_msg = "파일이 열려 있어 저장할 수 없습니다. Excel에서 닫은 뒤 다시 시도하세요."
            except Exception as exc:
            ...
            def on_done() -> None:
                ...
                messagebox.showinfo("완료", "Export 되었습니다.")
    ...
    def merge_data(self) -> pd.DataFrame:
        ...
        # prepare OCR summary grouped by coupon번호
